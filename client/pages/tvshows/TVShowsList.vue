<template>
  <div class="dashboard-container">
    <div id="item-list" v-for="(group, i) in moviesGroups" class="row">
        <poster v-for="movie in object.data.slice(i * moviesPerRow, (i + 1) * moviesPerRow)" :key="movie.id"  :type="'tv'" :data="movie" :active="selectPoster" @click.native="selectMovie(movie.id, i)"/>
        <Card :key="i" v-if="i == selectCard && Object.keys(movie).length" :type="'tv'" :object="movie" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    async selectMovie(id, card) {
      const {data} = await axios.get('/tvshows/detalis/'+id)
      this.movie = data
